...
class Swarm(Population):
    ...
    def uniform_crossover(self, parents, alpha=None):
    ...
    def 2points_crossover(self, parents, alpha=None):
        genes_number = len(parents[0]._weight)
        children = []
        for i in range(int(self._offspring_number / 2)):
            if self.verbose > 1:
    ...
    def 1point_crossover(self, parents, alpha=None):
        children = []
        for i in range(int(self._offspring_number / 2)):
            if self.verbose > 1:
                print('\t\t{}_th 2 childs'.format(i + 1))
    ...
    def generate_next_population(self):
        ...
        if self.verbose > 1:
            ...
            start_time = time()
        crossover_switcher = {
            '1point': self.1point_crossover,
            '2points': self.2points_crossover,
